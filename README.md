...
**Текущий статус:** основной collection-конвейер стабилизирован и проверен,
31.08.2026. OnlineTours различает успешную цену, отсутствие предложений и
технические ошибки; необязательный поиск дешёвой даты больше не может потерять
весь запрос. Владелец также подтвердил успешную конвертацию всех найденных
ссылок через Travelpayouts.
...
Последняя стабилизационная проверка: 31/31 unit-тестов, два ранее потерянных
live-запроса прошли 2/2, затем 28 последовательных запросов завершились без
технических ошибок. Полный повторный benchmark после этого патча не требуется
без отдельной причины.
...
включает поиск более выгодной даты. Этот этап необязателен: если график
процентов не загрузился, парсер отмечает `CHEAPEST_DATE_UNAVAILABLE` в логе и
продолжает поиск по исходной дате.

## Итоговые состояния обычного парсера

Каждый входной запрос сохраняется в `configs/collection_urls.txt` с одним
итоговым состоянием:

- `PRICE` — URL и цена успешно получены;
- `NO_RESULTS` — OnlineTours явно сообщил, что предложений нет;
- `PRICE_PARSE_ERROR` — выдача появилась, но цену нельзя надёжно распознать;
- `REQUEST_ERROR` — запрос не завершён из-за другой технической ошибки.

После ошибки пакет не ждёт `Enter` и переходит к следующему запросу. В конце
выводится сводка по состояниям и количеству fallback дешёвой даты.
Travelpayouts-конвертер обрабатывает только записи с URL; остальные статусы
пропускает целиком.
...
Ближайший практический шаг — отдельно проверить hotel-mode. Обычный collection
mode и Travelpayouts-конвертер уже приняты на реальных данных. Полный benchmark
не надо запускать после каждого изменения: сначала unit-тесты и один-два
целевых live-запроса.

Typed models, внутренний JSON transport, Travelpayouts API, HotelIQ и DealScore
остаются возможными будущими улучшениями, но не текущей срочной задачей.
